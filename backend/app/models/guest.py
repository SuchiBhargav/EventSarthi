...
class Guest(Base):
    ...
    planner_id = Column(
        UUID(as_uuid=True),
        ForeignKey("planners.planner_id"),
        nullable=False,
        index=True,
    )
    event_id = Column(
    ...
    tone_preference = Column(
        String(50), default=TonePreference.FRIENDLY.value, nullable=False
    )
    ...
    conversations = relationship(
        "Conversation", back_populates="guest", cascade="all, delete-orphan"
    )
    support_requests = relationship(
        "SupportRequest", back_populates="guest", cascade="all, delete-orphan"
    )
