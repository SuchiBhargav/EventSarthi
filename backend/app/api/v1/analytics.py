...
async def get_event_analytics(
    ...
    total_conversations = db.query(Conversation).filter(
        Conversation.event_id == event.event_id
    ).count()
    
    # Broadcast statistics
    total_broadcasts = db.query(Broadcast).filter(
        Broadcast.event_id == event.event_id
    ).count()
    ...
    return {
        ...
        "guest_analytics": {
            ...
            "attendance_rate": round((confirmed_guests / total_guests * 100) if total_guests > 0 else 0, 2)
        },
