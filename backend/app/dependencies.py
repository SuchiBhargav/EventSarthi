...
def get_current_planner(
    ...
    if planner is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Planner not found"
        )

    if planner.is_active is False:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Planner account is inactive"
        )
...
def get_current_active_planner(
    ...
    if current_planner.is_active is False:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Inactive planner"
        )
